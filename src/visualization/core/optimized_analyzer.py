                ...
                if cached_hash == current_hash:
                    ...
                    if cache_file.exists():
                        with open(cache_file, 'r') as f:
                            data = json.load(f)
                        entities = [CodeEntity.model_validate(e) for e in data["entities"]]
                        deps = [Dependency.model_validate(d) for d in data["dependencies"]]
...
class OptimizedAnalyzer(AbstractAnalyzer):
    """
    High-performance code analyzer with caching and parallel processing.
    """

    kind = AnalyzerKind.OPTIMIZED
